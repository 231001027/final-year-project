...
export default function FacultyLogin() {
  ...
  const [errors, setErrors] = useState<{ facultyId?: string; password?: string }>({});
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setIsLoading(true);
    const result = await loginFaculty(facultyId.trim(), password);
    setIsLoading(false);
      ...
      <div className="w-full max-w-md">
        ...
        <div className="bg-white dark:bg-slate-800 rounded-2xl shadow-xl p-8 border border-slate-200 dark:border-slate-700">
          <div className="text-center mb-8">
            ...
            <h1 className="text-2xl font-bold text-slate-900 dark:text-white">Faculty Login</h1>
            <p className="text-slate-500 dark:text-slate-400 mt-2">
              Access the faculty portal to manage projects
            </p>
          </div>
          ...
          <div className="mt-6 pt-6 border-t border-slate-200 dark:border-slate-700">
            ...
            <div className="bg-slate-50 dark:bg-slate-900 rounded-lg p-4 text-xs font-mono">
              <p className="text-slate-600 dark:text-slate-400">Faculty ID: <span className="text-blue-600 dark:text-blue-400">FAC001</span> (Dr.A.Kalaivani)</p>
              <p className="text-slate-600 dark:text-slate-400 mt-1">Password: <span className="text-blue-600 dark:text-blue-400">faculty123</span></p>
            </div>
          </div>
